...
const Child = () => {
    ...
    useEffect(() => {
        const fetchBlogs = async() => {
                ...
                try{
                    const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/Donationblogs/category/?search=เด็กโต`);
                    setBlogs(res.data);
                }
                catch(err){
        ...
        <div className='container mt-3'>
            ...
            <div className="nav-scroller py-1 mb-2">
                <nav className="nav d-flex justify-content-start">
                    ...
                    <Link className="p-2 text-muted" to='../categoryO/เด็กโต เสื้อผ้า'>เสื้อผ้า</Link>
                    <Link className="p-2 text-muted" to='../categoryO/เด็กโต รองเท้า'>รองเท้า</Link>
                    <Link className="p-2 text-muted" to='../categoryO/เด็กโต ของใช้'>ของใช้</Link>
                    <Link className="p-2 text-muted" to='../categoryO/เด็กโต อาหารและยา'>อาหารและยา</Link>
                    <Link className="p-2 text-muted" to='../categoryO/เด็กโต เงินบริจาค'>เงินบริจาค</Link>
